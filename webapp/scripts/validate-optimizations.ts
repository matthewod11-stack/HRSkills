...
import { analyzeRequiredFields, filterEmployeeFields, estimateTokenCount, type Employee } from '../lib/employee-context'
import { calculateTokenCost, calculateCacheSavings, calculateCacheEfficiency } from '../lib/performance-monitor'
...
function generateMockEmployees(count: number): Employee[] {
  const departments = ['Engineering', 'Product', 'Design', 'Sales', 'Marketing', 'Finance', 'HR', 'Operations']
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    performance_rating: i % 3 === 0 ? 'Outstanding' : i % 2 === 0 ? 'Exceeds Expectations' : 'Meets Expectations',
...
const testQueries = [
  ...
  { query: "What's the average salary by department?", category: "Compensation", expectedSavings: "High" },
  { query: "Who are the top performers in sales?", category: "Performance", expectedSavings: "Medium" },
  { query: "Analyze our diversity metrics", category: "DEI", expectedSavings: "Medium" },
  { query: "What's our turnover rate in the last year?", category: "Retention", expectedSavings: "High" },
  { query: "List all remote employees", category: "Directory", expectedSavings: "High" },
  { query: "Who reports to FirstName0 LastName0?", category: "Org Structure", expectedSavings: "Very High" },
  { query: "Show me employees hired in 2020", category: "Tenure", expectedSavings: "Medium" },
  { query: "What's the gender distribution across departments?", category: "DEI", expectedSavings: "Medium" },
  { query: "Count active employees in each location", category: "Headcount", expectedSavings: "Very High" },
]
...
const catalogCost = {
  ...
  cached: (catalogTokens / 1_000_000) * 0.30,
...
console.log(`   - Savings: $${(catalogCost.uncached - catalogCost.cached).toFixed(6)} per request (90%)`)
...
for (const test of testQueries) {
  ...
  const baselineContext = sampleEmployees.map(emp => {
    const fields = allFields.filter(f => f !== 'employee_id' && f !== 'full_name').map(f => `${f}: ${emp[f]}`).join(', ')
  ...
  const optimizedContext = optimizedEmployees.map(emp => {
    const fullName = emp.full_name || `${emp.first_name} ${emp.last_name}`
    const fields = requiredFields
      .filter(f => f !== 'employee_id' && f !== 'full_name' && f !== 'first_name' && f !== 'last_name' && f in emp)
      .map(f => `${f}: ${emp[f]}`)
      .join(', ')
...
queryResults.forEach((result, i) => {
  ...
  console.log(`   Tokens: ${result.baselineTokens.toLocaleString()} → ${result.optimizedTokens.toLocaleString()}`)
  console.log(`   Reduction: ${result.reduction.toFixed(1)}% (saved ${result.savedTokens.toLocaleString()} tokens)`)
  console.log('')
})
...
const filteringSavingsPerQuery = ((totalBaselineTokens - totalOptimizedTokens) / testQueries.length / 1_000_000) * 3
...
const baselineCostPerRequest =
  (baselineInputTokens / 1_000_000) * 3 +
  (baselineOutputTokens / 1_000_000) * 15
...
const optimizedCostPerRequest =
  ...
  (optimizedCachedTokens / 1_000_000) * 0.30 +
  (optimizedOutputTokens / 1_000_000) * 15

console.log('Per-Request Cost Analysis:')
console.log('')
console.log('BASELINE (No Optimizations):')
console.log(`   Input tokens: ${baselineInputTokens.toLocaleString()} @ $3/M = $${((baselineInputTokens / 1_000_000) * 3).toFixed(6)}`)
console.log(`   Output tokens: ${baselineOutputTokens.toLocaleString()} @ $15/M = $${((baselineOutputTokens / 1_000_000) * 15).toFixed(6)}`)
console.log(`   Total: $${baselineCostPerRequest.toFixed(6)}`)
console.log('')
console.log('OPTIMIZED (All Phases):')
console.log(`   Input tokens: ${optimizedInputTokens.toFixed(0)} @ $3/M = $${((optimizedInputTokens / 1_000_000) * 3).toFixed(6)}`)
console.log(`   Cached tokens: ${optimizedCachedTokens.toLocaleString()} @ $0.30/M = $${((optimizedCachedTokens / 1_000_000) * 0.30).toFixed(6)}`)
console.log(`   Output tokens: ${optimizedOutputTokens.toLocaleString()} @ $15/M = $${((optimizedOutputTokens / 1_000_000) * 15).toFixed(6)}`)
console.log(`   Total: $${optimizedCostPerRequest.toFixed(6)}`)
console.log('')
console.log(`SAVINGS PER REQUEST: $${(baselineCostPerRequest - optimizedCostPerRequest).toFixed(6)} (${((1 - optimizedCostPerRequest / baselineCostPerRequest) * 100).toFixed(1)}% reduction)`)
...
console.log(`   Savings: $${savingsMonthly.toFixed(2)} (${((savingsMonthly / baselineMonthly) * 100).toFixed(1)}% reduction)`)
...
console.log(`   Year 1 ROI: ${((savingsMonthly * 12 - implementationCost) / implementationCost * 100).toFixed(0)}%`)
