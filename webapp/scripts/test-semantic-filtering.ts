...
const testQueries = [
  ...
  },
  {
    ...
    expectedFields: ['employee_id', 'full_name', 'first_name', 'last_name', 'gender', 'ethnicity', 'department', 'level'],
    category: 'DEI Analysis'
  },
  {
    query: "What's our employee turnover rate?",
    expectedFields: ['employee_id', 'full_name', 'first_name', 'last_name', 'hire_date', 'termination_date', 'status'],
    category: 'Turnover Analysis'
  },
  {
    query: "Show me high performers in product",
    expectedFields: ['employee_id', 'full_name', 'first_name', 'last_name', 'performance_rating', 'department'],
    category: 'Performance Review'
  },
  {
    query: "List all employees with their titles and locations",
    expectedFields: ['employee_id', 'full_name', 'first_name', 'last_name', 'job_title', 'location'],
    category: 'Basic Directory'
  },
  {
    query: "What's the tenure distribution across teams?",
    expectedFields: ['employee_id', 'full_name', 'first_name', 'last_name', 'hire_date', 'department', 'manager_name'],
...
for (let i = 0; i < testQueries.length; i++) {
  ...
  const baselineContext = mockEmployees.map(emp => {
...
const totalReduction = ((totalBaselineTokens - totalOptimizedTokens) / totalBaselineTokens * 100).toFixed(1)
