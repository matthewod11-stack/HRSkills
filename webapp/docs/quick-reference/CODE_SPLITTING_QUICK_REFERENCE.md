...
const Dialog = dynamic(() => import('./Dialog').then((mod) => ({ default: mod.Dialog })), {
...
export default function Home() {
  ...
  return (
    <div>
      <button onClick={() => setDialogOpen(true)}>Open Details</button>

      <Suspense fallback={<DialogSkeleton />}>
        <MetricDetailsDialog isOpen={dialogOpen} onClose={() => setDialogOpen(false)} />
      </Suspense>
    </div>
  );
}
