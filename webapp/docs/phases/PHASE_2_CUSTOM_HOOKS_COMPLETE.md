...
```tsx
const { currentPage, totalPages, getCurrentPageItems, nextPage, previousPage, goToPage } =
  usePagination({
...
const {
  ...
  isError,
  error,
} = useAsync(() => fetch(`/api/users/${userId}`).then((res) => res.json()), true);
...
function SearchBar() {
  ...
  const { data, isLoading } = useAsync(() => searchAPI(debouncedSearch), false);
