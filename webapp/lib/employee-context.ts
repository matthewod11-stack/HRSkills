...
export function filterRelevantEmployees(
  ...
  const departments = [
    'engineering', 'eng', 'product', 'design', 'marketing',
    'sales', 'finance', 'hr', 'operations', 'ops', 'legal'
  ]
  ...
  for (const title of titlePatterns) {
    if (queryLower.includes(title)) {
      const filtered = employees.filter(emp =>
        emp.job_title?.toLowerCase().includes(title)
      )
      if (filtered.length > 0) {
...
export function generateEmployeeContext(
  ...
  const employeeSummary = activeEmployees.map((emp: Partial<Employee>) => {
    const fullName = emp.full_name || (emp.first_name && emp.last_name ? `${emp.first_name} ${emp.last_name}` : 'Unknown')
