...
export function detectWorkflow(context: DetectionContext): WorkflowMatch {
  ...
  const workflowBoost = currentWorkflow && currentWorkflow !== 'general' ? CONVERSATION_HISTORY_BOOST : 0
  ...
  if (scores.length === 0 || scores[0].score < CONFIDENCE_THRESHOLD) {
    return {
      ...
      confidence: 0,
      matchedTriggers: [],
      contextFactors: scores.length === 0 ? ['no_patterns_matched'] : ['confidence_below_threshold']
    }
  }
 ...
 */
export function validateDetection(testCases: Array<{
  message: string
