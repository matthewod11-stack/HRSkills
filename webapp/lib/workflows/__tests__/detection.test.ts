...
describe('Workflow Detection', () => {
  ...
  describe('GENERAL workflow fallback', () => {
    ...
    it('should fallback to general for non-HR queries', () => {
      const testCases = ['What is the weather today?', 'Tell me a joke', 'How do I cook pasta?'];
    ...
    it('should achieve 95%+ accuracy on test set', () => {
      ...
      if (validation.failures.length > 0) {
        ...
        validation.failures.forEach((failure) => {
          ...
          console.log(
            `    Expected: ${failure.expected}, Got: ${failure.actual} (confidence: ${failure.confidence})`
          );
        });
      }
